...
export function PlanView({ plan, phases, userId, role }: PlanViewProps) {
  ...
  useEffect(() => {
    ...
    if (taskParam && tasks.length > 0) {
      const exists = tasks.find((t) => t.id === taskParam);
      if (exists) {
        ...
        setTimeout(() => {
          const el = document.getElementById(`task-${taskParam}`);
          if (el) {
    ...
    <>
      <div className="mx-auto max-w-4xl space-y-6 pb-24">
        ...
        <div className="flex items-start gap-4">
          ...
          {role === "mentor" && (
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 shrink-0 transition-colors"
            >
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_auto]">
          ...
          <div className="flex items-center rounded-xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
            <ProgressGauge
      ...
      {chatOpen && chatMinimized && (
        <button
          ...
          className="fixed bottom-0 right-6 z-40 flex h-12 w-72 items-center gap-2 rounded-t-lg bg-slate-900 px-4 text-white shadow-lg hover:bg-slate-800 transition-colors"
